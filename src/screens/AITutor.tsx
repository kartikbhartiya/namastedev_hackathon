...
import { ArrowLeft, Bot, Send, Plus, Trash2, MessageSquare, Sparkles, User, Zap } from "lucide-react";
...
export function AITutor() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    // Update title dynamically if it's the first user input
    ...
    try {
      // Build context from session history (last 8 messages to keep payload small & fast)
      ...
      // Update session with empty assistant template
      ...
      // Save final response state to localStorage
      ...
      const errorSession: ChatSession = {
        ...
        messages: [
          ...
          {
            role: "assistant",
            content: "Failed to generate tutor response. Please check your active API keys under settings."
          }
        ]
      };
  ...
  return (
    <div className="min-h-screen bg-[#040404] text-white flex flex-col select-none">
      ...
      <header className="border-b border-white/5 bg-[#090909] py-4 z-50">
        <div className="container mx-auto px-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-lg font-bold flex items-center gap-2">
                <Bot className="w-5 h-5 text-[#ff6c37]" />
                Orbit AI Tutor
              </h1>
              <p className="text-xs text-neutral-500">Your interactive CS syllabus coach.</p>
            </div>
          </div>
          ...
          <Button 
            ...
            size="sm"
            className="bg-[#ff6c37] hover:bg-[#ff8454] text-black font-bold flex items-center gap-1.5"
          >
      ...
      <div className="flex-1 flex overflow-hidden container mx-auto px-6 py-6 gap-6 max-w-6xl">
        
        {/* Left Sessions Sidebar */}
        <aside className="w-64 border border-white/5 rounded-xl bg-[#0b0b0b] overflow-hidden flex flex-col shrink-0">
        ...
        <main className="flex-1 border border-white/5 rounded-xl bg-[#0b0b0b] overflow-hidden flex flex-col relative">
              ...
              activeSession.messages.map((m, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={cn(
                      "flex gap-4 p-4 rounded-xl border max-w-[85%] transition-all",
                      isAssistant
          ...
          <form onSubmit={handleSend} className="p-4 border-t border-white/5 bg-[#0e0e0e] flex gap-3 z-10 shrink-0">
            <Input
              placeholder={isResponding ? "Orbit is analyzing..." : "Ask Orbit anything (e.g. how do B-Trees balance?)"}
              value={input}
              onChange={(e) => setInput(e.target.value)}
              className="bg-transparent border-white/5 ring-0 focus-visible:ring-0 focus-visible:ring-offset-0 placeholder:text-neutral-600 text-sm h-12 flex-1"
              disabled={isResponding || !activeSessionId}
            />
            <Button
              type="submit"
              disabled={isResponding || !input.trim() || !activeSessionId}
              className="h-12 w-12 rounded-lg bg-[#ff6c37] hover:bg-[#ff8454] text-black flex items-center justify-center shrink-0 transition-colors"
            >
